...
function getRandomNumbers() {
    ...
    for (let i = numbers.length-1; i>=0; i--) {
        ...
        [numbers[i], numbers[randomIndex]] = [numbers[randomIndex], numbers[i]]; //деструктуризация массива()создание временного массива)
...
function validate(grid, row, column, value) {
    return validateColumn(grid, row, column, value) && validateRow(grid, row, column, value) && validateBox(grid, row, column, value);
}
...
function validateBox(grid, row, column, value) {
    ...
    let boxStartColumn = column - column % BOX_SIZE;

    for(let iRow = boxStartRow; iRow < boxStartRow + BOX_SIZE; iRow++){
        for (let iColumn = boxStartColumn; iColumn< boxStartColumn + BOX_SIZE; iColumn++) {
...
function removeCells(grid) {
 const DIFFICULTY = 30;
 const resultGrid = [...grid].map(row => [...row]); //копирование всех ячеек двумерного массива с помощью map;
